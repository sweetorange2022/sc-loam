...
// 3D点云在xy两个维度上以极坐标表示，在径向上可以分割可以划分出很多同心圆环ring
// 在360圆周上进行分割可以划分出很多同心扇形sector，两者交叉形成的就是很多个同心半圆环，每个区域用该区域所有点云高度的最大值来表示，
// 这就是ScanContext，它将3D点云压缩成了二维的矩阵;
...
MatrixXd SCManager::makeRingkeyFromScancontext( Eigen::MatrixXd &_desc )
{
     ...
     * summary: rowwise mean vector
     makeRingkeyFromScancontext是将每一个完整的ring用一个数字来代替，进而用一个向量来描述整个点云，
                                                                      方法就是，对于一个完整ring上不同sector区域内的值取均值即可;
    */
...
MatrixXd SCManager::makeSectorkeyFromScancontext( Eigen::MatrixXd &_desc )
{
     ...
     * summary: columnwise mean vector
     makeSectorkeyFromScancontext是想用一个数字来表述整个sector，
     方法就是将同一个sector内不同ring内的值取均值。
    */
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
     ...
     */
    // 之所以使用ring key是因为,ring代表了一周,即便角度相差很大对应ring的key(均值)也相差不大
     ...
     */
    if( min_dist < SC_DIST_THRES )//如果某帧经过最优偏移后的距离足够小
    {
        loop_id = nn_idx; 
